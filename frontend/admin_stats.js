...
async function loadStats() {
    ...
    renderTable(
        "tableWaiter",
        ["Bedienung", "Wechselgeld Start", "Orders", "Gesamtsumme Orders", "Offen", "Alles kassiert"],
        (data.by_waiter || []).map(w => [
            w.waiter_name || "-",
            money(w.opening_cash),
            w.orders || 0,
    ...
    const personalRows = (data.by_station_product || []).filter(r => Number(r.qty_internal || 0) > 0);
    renderTable(
        "tablePersonal",
        ["Theke/Quelle", "Artikel", "Menge Personal", "Menge bezahlt", "Umsatz bezahlt"],
        personalRows.map(r => [
            r.station_name || "–",
            r.product_name || "-",
            String(r.qty_internal || 0),
            String(r.qty_paid || 0),
            money(r.revenue_paid),
        ])
    );
    ...
    renderTable(
        "tableVatTotals",
        ["MwSt-Satz", "Brutto (VK)", "Netto-Umsatz", "MwSt-Betrag"],
        (data.vat_totals || []).map(v => [
            String(v.vat_rate) + " %",
            money(v.brutto_sum),
            money(v.netto_sum),
            money(v.mwst_sum),
    ...
    renderTable(
        "tableVatLines",
        ["Pos.", "VK/Stück", "MwSt", "Brutto gesamt", "Netto", "MwSt-Anteil"],
        (data.vat_lines || []).map(v => {
            const q = Number(v.qty_paid || 0);
            const up = Number(v.unit_price || 0);
            const pos = `${q}× ${v.name || "-"} à ${euro(up)}`;
            return [
                pos,
                money(up),
                String(v.vat_rate) + " %",
    ...
    renderTable(
        "tableCancellations",
        ["Grund", "Anzahl", "Menge", "Wert"],
        (data.by_cancellation_reason || []).map(r => [
            r.reason || "-",
            r.cancellations_count || 0,
            r.cancellations_qty || 0,
            money(r.cancellations_amount),
        ])
    );
...
function printFullReport() {
    ...
    const cashier = document.getElementById("tableCashier");
    const tabs = document.getElementById("tableTabs");
    const categories = document.getElementById("tableCategories");
    const vatTotals = document.getElementById("tableVatTotals");
    const vatLines = document.getElementById("tableVatLines");
    const cancellations = document.getElementById("tableCancellations");
    ...
    w.document.write(`
        ...
        <h2>Kassenstand (Bedienung/Theke)</h2>
        ${cashier ? cashier.outerHTML : ""}
        <h2>Deckel</h2>
        ${tabs ? tabs.outerHTML : ""}
        <h2>Kategorien</h2>
        ${categories ? categories.outerHTML : ""}
        <h2>Umsatz nach MwSt</h2>
        ${vatTotals ? vatTotals.outerHTML : ""}
        <h2>Positionen mit MwSt-Aufschlüsselung</h2>
        ${vatLines ? vatLines.outerHTML : ""}
        <h2>Stornos</h2>
        ${cancellations ? cancellations.outerHTML : ""}
