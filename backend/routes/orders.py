...
from app_mode import is_single_terminal_mode, initial_station_status
from db import get_db_connection, sql_tab_open_balance

orders_bp = Blueprint('orders', __name__)


def _ensure_order_station_status(conn, order_id: int, station_id: int) -> None:
    exists = conn.execute(
        "SELECT 1 FROM order_station_status WHERE order_id = ? AND station_id = ?",
        (order_id, station_id),
    ).fetchone()
    if not exists:
        conn.execute(
            ...
            VALUES (?, ?, ?)
            """,
            (order_id, station_id, initial_station_status(conn)),
        )
...
def _order_item_prep_detail(
    ...
    """
    if is_single_terminal_mode(conn):
        return True, {}
    ...
    for s in stations:
        ...
        # Theken-Direktbestellungen (source=station) legen nur eine OSS-Zeile pro Order an.
        # Mehrere Stationen koennen dieselbe Kategorie bedienen — ohne OSS-Zeile war
        # diese Station an der Order nicht beteiligt, dann nicht gegen "ready" pruefen.
        if not oss:
            continue
        st = str(oss["status"] or "").lower()
        if st != "ready":
            return False, {
                "station_id": sid,
...
def station_display(station_id):
    ...
    if is_single_terminal_mode(conn):
        conn.close()
        return jsonify({"slots": [None] * 15, "waiting": 0})
...
def submit_order():
    """Atomarer Bestell-Endpoint: Order + Positionen in einer Transaktion."""
    data = request.json or {}
    ...
    source = "guest_qr" if waiter_id is None else "waiter"
    row = conn.execute(
        "SELECT MAX(order_number) AS max_nr FROM orders WHERE event_id = ?",
        (event_id,),
    ).fetchone()
    next_number = (row["max_nr"] or 0) + 1
    ...
    cur = conn.execute(
        """
        INSERT INTO orders (event_id, order_number, table_id, waiter_id, status, source, source_station_id)
        VALUES (?, ?, ?, ?, 'new', ?, NULL)
        """,
        (event_id, next_number, table_id, waiter_id, source),
    )
    ...
    try:
        for item in items:
            ...
            product = conn.execute(
                """
                SELECT category_id, price, name, event_id, COALESCE(vat_rate, 19) AS vat_rate
                FROM products
                WHERE id = ?
                """,
                (product_id,),
            ...
            stations = conn.execute(
                ...
                WHERE category_id = ?
                  AND event_id = ?
                """,
                (product["category_id"], product["event_id"]),
            ).fetchall()
            ...
            if existing:
                ...
                conn.execute(
                    ...
                    INSERT INTO order_items (
                        ...
                        quantity_total, quantity_open, quantity_paid, vat_rate
                    )
                    VALUES (?, ?, ?, ?, '', ?, ?, 0, ?)
                    """,
                    (order_id, product_id, product["price"], product["name"], qty, qty, int(product["vat_rate"])),
                )
            ...
            for s in stations:
                ...
                if not exists:
                    _ensure_order_station_status(conn, order_id, s["station_id"])
    except LookupError:
        conn.rollback()
        conn.close()
...
@orders_bp.route("/orders/<int:order_id>/cancel-item", methods=["POST"])
def cancel_item(order_id):
    ...
    if int(item["quantity_open"] or 0) < quantity:
        conn.close()
        return jsonify({"error": "quantity exceeds open items"}), 400
    ...
    """, (
        order_id, order_item_id, int(order_row["event_id"]), reason, quantity,
        unit_price, amount, created_by_role, created_by_user_id, created_by_station_id
    ))
    conn.execute("""
        UPDATE order_items
...
def pay_internal(order_id):
    ...
    if not is_single_terminal_mode(conn):
        for r in open_rows:
...
def station_create_order(station_id):
    ...
    for item in items:
        ...
        if terminal:
            product = conn.execute("""
                SELECT p.id, p.name, p.price, p.category_id, COALESCE(p.vat_rate, 19) AS vat_rate
                FROM products p
                WHERE p.id = ?
                  AND p.event_id = ?
                  AND p.active = 1
        ...
        _ensure_order_station_status(conn, order_id, station_id)
        station_exists = True
