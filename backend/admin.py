...
from routes.icons import _normalize_icon_fields, enrich_product_icon
from db import (
    get_db_connection,
    get_active_event_id,
...
def _vat_split_brutto(brutto: float, rate_pct: int) -> tuple[float, float]:
    """VK brutto (inkl. MwSt) -> (netto_umsatz, mwst_betrag)."""
    factor = 1.0 + float(rate_pct) / 100.0
    netto = brutto / factor
    return netto, brutto - netto


def _parse_opening_cash(raw_value) -> float:
    s = str(raw_value if raw_value is not None else "0").strip().replace(",", ".")
    try:
        v = float(s)
...
def _parse_optional_cash(raw_value):
    s = str(raw_value if raw_value is not None else "").strip().replace(",", ".")
    if s == "":
        return None
    try:
        v = float(s)
...
def _read_feature_flags(conn):
    rows = conn.execute(
        "SELECT key, value FROM app_settings WHERE key LIKE 'beta_%' ORDER BY key ASC"
    ).fetchall()
    out = {}
    for r in rows:
        key = str(r["key"])
        val = str(r["value"] or "0").strip().lower()
        out[key] = val in ("1", "true", "yes", "on")
    return out


def _read_app_settings(conn):
    flags = _read_feature_flags(conn)
    row = conn.execute(
        "SELECT value FROM app_settings WHERE key = 'single_terminal_mode'"
    ).fetchone()
    val = str(row["value"] or "0").strip().lower() if row else "0"
    return {
        "features": flags,
        "single_terminal_mode": val in ("1", "true", "yes", "on"),
    }


def _save_app_settings(conn, data):
    ...
    if "single_terminal_mode" in data:
        v = "1" if bool(data.get("single_terminal_mode")) else "0"
        conn.execute(
            "INSERT INTO app_settings (key, value) VALUES ('single_terminal_mode', ?) "
            "ON CONFLICT(key) DO UPDATE SET value=excluded.value",
            (v,),
        )
    ...
    if request.method == "POST":
        ...
        if template_event_id:
            ...
            prod_rows = conn.execute("""
                SELECT id, name, price, category_id, display_category_id, active,
                       COALESCE(vat_rate, 19) AS vat_rate,
                       COALESCE(icon_type, 'none') AS icon_type,
                       icon_ref
                FROM products
                WHERE event_id = ?
                ORDER BY id ASC
            ...
            for p in prod_rows:
                dcid = p["display_category_id"]
                new_dcid = cat_map.get(dcid) if dcid is not None else None
                conn.execute("""
                    INSERT INTO products (
                        event_id, name, price, category_id, display_category_id, active, vat_rate,
                        icon_type, icon_ref
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    ...
                    new_dcid,
                    p["active"],
                    int(p["vat_rate"]) if p["vat_rate"] is not None else 19,
                    p["icon_type"] or "none",
                    p["icon_ref"],
                ))
            ...
            user_rows = conn.execute("""
                SELECT id, name, pin, role, active, COALESCE(opening_cash, 0) AS opening_cash, closing_cash
                FROM users
                WHERE event_id = ?
                ORDER BY id ASC
            """, (template_event_id,)).fetchall()
            ...
            for u in user_rows:
                ...
                """, (
                    ...
                    float(u["opening_cash"] or 0), u["closing_cash"]
                ))
...
def clear_event_orders():
    """Deaktiviert im Web-Admin — nur GastroSystem Control Panel (/host/*)."""
    return jsonify(
        {
            "error": "forbidden",
            "hint": "Bestellungen/Event-Daten nur ueber GastroSystem Steuerung (Host-Panel).",
        }
    ), 403
...
def event_stats():
    ...
    cancellations_sum = conn.execute("""
        SELECT
            ...
            COALESCE(SUM(quantity), 0) AS cancellations_qty,
            COALESCE(SUM(amount), 0) AS cancellations_amount
    ...
    by_cancellation_reason = conn.execute("""
        SELECT
            reason,
            COUNT(*) AS cancellations_count,
            COALESCE(SUM(quantity), 0) AS cancellations_qty,
        ...
        WHERE event_id = ?
        GROUP BY reason
        ORDER BY cancellations_amount DESC, cancellations_qty DESC
        LIMIT 20
    """, (event_id,)).fetchall()
    ...
    vat_agg = conn.execute("""
        SELECT
            COALESCE(oi.vat_rate, p.vat_rate, 19) AS vat_rate,
            SUM(oi.quantity_paid * COALESCE(oi.unit_price, p.price)) AS brutto_sum
        FROM order_items oi
        JOIN orders o ON o.id = oi.order_id
        JOIN products p ON p.id = oi.product_id
        WHERE o.event_id = ?
        GROUP BY COALESCE(oi.vat_rate, p.vat_rate, 19)
        ORDER BY vat_rate ASC
    """, (event_id,)).fetchall()

    vat_totals_out = []
    for r in vat_agg:
        vr = int(r["vat_rate"] or 19)
        b = float(r["brutto_sum"] or 0)
        netto, mwst = _vat_split_brutto(b, vr)
        vat_totals_out.append({
            "vat_rate": vr,
            "brutto_sum": b,
            "netto_sum": netto,
            "mwst_sum": mwst,
    ...
    vat_detail_rows = conn.execute("""
        ...
        WHERE o.event_id = ?
        GROUP BY
            COALESCE(oi.product_name, p.name),
            COALESCE(oi.unit_price, p.price),
            COALESCE(oi.vat_rate, p.vat_rate, 19)
        HAVING SUM(oi.quantity_paid) > 0
        ORDER BY name COLLATE NOCASE ASC, vat_rate ASC
        LIMIT 500
    """, (event_id,)).fetchall()

    vat_lines_out = []
    for r in vat_detail_rows:
        vr = int(r["vat_rate"] or 19)
        b = float(r["brutto_sum"] or 0)
        netto, mwst = _vat_split_brutto(b, vr)
        vat_lines_out.append({
            "name": r["name"],
            "vat_rate": vr,
            "qty_paid": int(r["qty_paid"] or 0),
    ...
    return jsonify({
        ...
        "cancellations": dict(cancellations_sum),
        ...
        "by_cancellation_reason": [dict(r) for r in by_cancellation_reason],
        "by_station_product": [dict(r) for r in by_station_product],
        "tabs": [dict(r) for r in tab_rows],
        "vat_totals": vat_totals_out,
        "vat_lines": vat_lines_out,
    })
...
def products():
    ...
    if request.method == "POST":
        data = request.json
        dcid = data.get("display_category_id")
        if dcid is not None and dcid != "":
            try:
                dcid = int(dcid)
            except (TypeError, ValueError):
                dcid = None
        ...
        if vr not in (7, 19):
            vr = 19
        icon_type, icon_ref = _normalize_icon_fields(
            data.get("icon_type"), data.get("icon_ref")
        )
        conn.execute("""
            INSERT INTO products (
                event_id, name, price, category_id, display_category_id, vat_rate,
                icon_type, icon_ref
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            event_id, data["name"], data["price"], data["category_id"], dcid, vr,
            icon_type, icon_ref,
        ))
        conn.commit()
    ...
    rows = conn.execute("""
        SELECT
            ...
            p.name,
            p.price,
            COALESCE(p.vat_rate, 19) AS vat_rate,
            c.name AS category,
            p.category_id,
...
def update_product():
    ...
    conn.execute("""
        UPDATE products
        SET name = ?, price = ?, category_id = ?, display_category_id = ?, vat_rate = ?,
            icon_type = ?, icon_ref = ?
...
def users():
    ...
    if request.method == "POST":
        data = request.json
        opening_cash = _parse_opening_cash(data.get("opening_cash"))
        conn.execute("""
            INSERT INTO users (event_id, name, pin, role, active, opening_cash)
            VALUES (?, ?, ?, ?, 1, ?)
        """, (event_id, data["name"], data["pin"], data["role"], opening_cash))
        conn.commit()
...
def update_user():
    ...
    opening_cash = _parse_opening_cash(data.get("opening_cash"))
    closing_cash = _parse_optional_cash(data.get("closing_cash"))

    if not user_id or not name or not pin:
        conn.close()
        return jsonify({"error": "id, name, pin required"}), 400

    conn.execute(
        """
        UPDATE users
        SET name = ?, pin = ?, opening_cash = ?, closing_cash = ?
        WHERE id = ? AND event_id = ?
        """,
        (name, pin, opening_cash, closing_cash, user_id, event_id),
    )
    conn.commit()
    conn.close()
    return jsonify({"status": "ok"})
...
def public_features():
    conn = get_db_connection()
    settings = _read_app_settings(conn)
    conn.close()
    return jsonify(settings)


@admin_bp.route("/admin/features", methods=["GET", "POST"])
def admin_features():
    conn = get_db_connection()

    if request.method == "POST":
        data = request.json or {}
        if data.get("features") is not None and not isinstance(data.get("features"), dict):
            conn.close()
            return jsonify({"error": "features must be object"}), 400
        _save_app_settings(conn, data)
        conn.commit()
